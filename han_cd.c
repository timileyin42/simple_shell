...
void cd_home(bash_shell *shell_op)
{
	...
	if (!home)
	{
		set_env("OLDPWD", cd_home, shel_op);
		free(cd_home);
		return;
	}
	if (directory_check(home) == -1)
	{
		error_fun(shell_op, 2);
		free(cd_home);
		return;
...
void cd_prev_dir(bash_shell *shell_op)
{
	...
	if (directory_check(cd_prev_dir) == -1)
